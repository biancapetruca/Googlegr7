...
while numar_incercari <= 2:
    ...
    if litera_incercata in cuvant_initial:
        ...
        if numar_incercari == 3:
             print(f"Ai pierdut! Cuvant initial: {cuvant}")
             break
        print(f"Mai ai {3 -numar_incercari} incercari. Ai incercat deja aceasta litera. Litrele incercate sunt: {','.join(set_litere_incercate)}")
